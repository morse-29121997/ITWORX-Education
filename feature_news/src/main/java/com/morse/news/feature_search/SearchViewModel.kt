...
import com.morse.news.feature_news.view_saved_news.SavedEvent
...
sealed class SearchEvent {
    data class OnWatchLaterSelected(val new: New, val isAdded: Boolean) : SearchEvent()
...
class SearchViewModel @Inject constructor(
    ...
    var searchState: MutableStateFlow<SearchState> = MutableStateFlow(SearchState.Empty)
    ...
    private fun update (function : (SearchState) -> SearchState) = searchState.update(function)

    fun onEvent(event: SearchEvent) {
        when (event) {
            is SearchEvent.OnWatchLaterSelected -> {
                if (event.isAdded) {
                    ...
                    ourState.copy(
                        headlineNews = headlineNewsUseCase.invoke(
                            ...
                            it.map {
                                val new = it.asUi()
                                new.apply {
                                    isWatchedLater.value = ourState.watchLater.count{ it.url == url }  > 0
                                }
                                new
                            }
                        },
